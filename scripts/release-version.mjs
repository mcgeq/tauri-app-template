...
const versionArg = cliArgs.find((arg) => !arg.startsWith("--"));
const dryRun = cliArgs.includes("--dry-run");
const noPush = cliArgs.includes("--no-push");
const noLint = cliArgs.includes("--no-lint");
const noBuild = cliArgs.includes("--no-build");
const noChangelog = cliArgs.includes("--no-changelog");
...
function getCliLanguage() {
  ...
  throw new Error(
    messages
      ...
      : "Invalid --lang value. Use zh or en.",
  );
}
...
function t(key, vars = {}) {
  ...
  if (typeof value !== "string") {
    throw new TypeError(`Missing translation key: ${key}`);
  }
...
function parseVersion(version) {
  ...
  if (!match) {
    throw new Error(
      t("releaseVersion.errors.unsupportedVersion", { version }),
    );
  }
  return {
...
async function chooseVersion(currentVersion) {
  ...
  const choice = await ask(
    t("releaseVersion.prompts.enterChoice", {
...
function ensureMainBranch() {
  ...
  if (branch !== "main") {
    throw new Error(
      t("releaseVersion.errors.branchNotMain", { branch }),
    );
  }
  return branch;
}
...
async function loadMessages() {
  ...
  const language = cliLanguage
    ?? (envLang.toLowerCase().startsWith("zh") ? "zh" : "en");
...
function runLint() {
  console.log("\n> pnpm run lint");
  execSync("pnpm run lint", { cwd: rootDir, stdio: "inherit" });
}
...
function runChangelog(version) {
  console.log(`\n> node scripts/changelog.mjs ${version} --force`);
  execSync(`node scripts/changelog.mjs ${version} --force`, {
    cwd: rootDir,
    stdio: "inherit",
  });
}
...
async function main() {
  try {
    ...
    const hasVersionArg = Boolean(versionArg);
    const nextVersion = hasVersionArg
      ? (parseVersion(versionArg), versionArg)
      : null;

    if (dryRun) {
      console.log(`[dry-run] release version: ${nextVersion ?? "(interactive)"}`);
      console.log(`[dry-run] flags:${noPush ? " --no-push" : ""}${noLint ? " --no-lint" : ""}${noBuild ? " --no-build" : ""}${noChangelog ? " --no-changelog" : ""}`);
      console.log("");

      if (!hasVersionArg) {
        const { packageVersion: currentVersion } = await loadCurrentVersions();
        console.log(`current version: ${currentVersion}`);
        console.log("(interactive mode — run with a version argument to skip prompts)");
        console.log("");
      }
      ...
      if (!noChangelog) console.log("  - CHANGELOG.md");
      console.log("commands to run:");
      if (!noLint) console.log("  - pnpm run lint");
      if (!noBuild) console.log("  - pnpm run build");
      if (!noChangelog) console.log(`  - node scripts/changelog.mjs ${nextVersion ?? "<version>"} --force`);
      console.log(`  - git commit -m "chore: release v${nextVersion ?? "<version>"}"`);
      console.log(`  - git tag v${nextVersion ?? "<version>"}`);
      if (!noPush) {
    ...
    const next = nextVersion ?? (await chooseVersion(currentVersion));
    parseVersion(next);
    ...
    console.log(
      t("releaseVersion.messages.summaryNextVersion", { version: next }),
    );
    console.log(t("releaseVersion.messages.summaryTag", { tag: tagName }));

    if (!hasVersionArg) {
      const confirmed = await askYesNo(
        "releaseVersion.prompts.confirmRelease",
      );
      if (!confirmed) {
    ...
    await updateVersions(next);

    if (!noChangelog) runChangelog(next);
    ...
    if (!noChangelog) addPaths.push("CHANGELOG.md");
    runInteractive(`git add ${addPaths.join(" ")}`);

    const stagedFiles = run("git diff --cached --name-only")
      .split(/\r?\n/)
      .filter(Boolean);
    for (const f of addPaths) {
      if (!stagedFiles.includes(f)) {
        throw new Error(
          `Expected ${f} to be staged but it was not. Staged: ${stagedFiles.join(", ")}`,
        );
      }
    }

    runInteractive(`git commit -m "chore: release ${next}"`);
    ...
    if (noPush) {
      ...
      const shouldPush = hasVersionArg || (await askYesNo("releaseVersion.prompts.pushNow"));
      if (shouldPush) {
        ...
        console.log(t("releaseVersion.messages.pushBranchLater", { branch }));
        console.log(
          t("releaseVersion.messages.pushTagLater", { tag: tagName }),
        );
      }
    }
  } catch (error) {
